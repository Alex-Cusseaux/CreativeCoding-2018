...
void ofApp::update(){
    ...
    threshold = ofMap(ofGetMouseX(), 0, ofGetWidth(), 0, 100);
    grabber.update();
    ...
    if (grabber.isFrameNew())
    {
        for (int x = 0; x < w; x++)
        {
            for (int y = 0; y < h; y++)
            {
                //get color of current pix
                ofColor currentColor = grabber.getPixels().getColor(x, y);
                
                //grab color of prev pix at same location
                ...
                {
                    // a lot has changed at that pix location
                    // so draw it out in diffPixels
                    diffPixels.setColor(x, y, currentColor);
                } else {
